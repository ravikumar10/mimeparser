...
eSafe firmy Aladdin to bardzo elastyczne i ³atwe w zarz±dzaniu rozwi±zanie dla
mniejszych i wiêkszych systemów informatycznych. Pozwala na instalacjê w wielu
konfiguracjach w zale¿no¶ci od potrzeb klienta. Sam produkt eSafe zawiera
w sobie:
\begin{itemize}
  \item eSafe Gateway - wprowadza dodatkowo warstwê bezpieczeñstwa. W
  przyk³adowym modelu mo¿e ona byæ umieszczona za firewallem przed serwerem
  proxy. Chroni sieæ przed atakami, wirusami, botami czy trojanami.
  \item eSafe Web - ochrona przed wszystkimi atakami zwi±zanymi z technologiami
  webowymi. Poza standardow± ochron± przed wirusami, oprogramowaniem ¶ledz±cym
  u¿ytkowników (spyware) czy filtrowaniem adresów URL, rozwi±zanie to dostarcza
  chocia¿by blokowanie znanych exploitów, politykê bezpieczeñstwa obiektów
  ActiveX czy blokowanie stron HTML zawieraj±cymi zdefiniowane s³owa kluczowe
  \item eSafe Web SSL - analiza szyfrowanych zawarto¶ci webowych czyli m.in.
  inspekcja szyfrowanego ruchu (HTTPS, SSL, TLS), automatyczne uniewa¿nianie
  cerfytikatów na podstawie CRL (Certificate Revocation List) czy obs³uga
  blacklist i whitelist dla administratorów.
  \item eSafe Mail - kompleksowa ochrona wiadomo¶ci email
  \item eSafe Modules - dodatkowe modu³u rozszerzaj±ce oprogramowanie eSafe
  takie jak:
  \begin{itemize}
    \item AppliFilter - monitoruje i blokuje nieautoryzowany ruch internetowy
    aplikacji z grup takich jak P2P (peer-to-peer), IM (instant messaging) czy
    Spyware.
    \item URL Filtering - monitoruje i blokuje dostêp do zdefiniowanych jako
    nieodpowiednie stron internetowych
    ...
    wynosi 0,05\%.
    \item SpywareNeutralizer - skaner oprogramowania typu spyware
    \item Advanced Reporter - modul do tworzenia raportów na podstawie logów z
    eSafe
  \end{itemize}
\end{itemize}
...
Pakietu E-mail Filter firmy Surfcontrol to system kompleksowego zarz±dzania
poczt± elektroniczn±. Dostêpny jest jako samodzielna brama SMTP lub jako
...
S± nimi:
\begin{itemize}
  \item Anti-Spam Agent (poprzednio: RiskFilter) - chroni firmê przed spamem
...
Amavisd-new to wysokiej jako¶ci interfejs pomiêdzy MTA, a filtrami zawarto¶ci:
skanerem wirusów i filtrem spamu. Oprogramowanie napisane jest w jêzyku Perl.
...
\begin{enumerate}
  ...
  \item Wiadomo¶ci z kolejki wyjmowane s± i przekazywane do analizatora.
...
niespasowania danego znaku b±d¼ dopsaowania ca³ego wzorca stosuje siê dwie
funkcje z wcze¶niej wyliczonymi warto¶ciami. S± t±: ''good-suffix shift'' oraz
''bad-character shift''. ³anie na przyk³adach. Za³ó¿my, ¿e czê¶æ wzorca i
teskstu zaznaczona jako u pasuje do siebie. Natomiast na kolejnej pozycji wystêpuje
ró¿nica. W zale¿no¶ci od sytuacji mo¿emy przesun±æ siê w poszukiwaniu o ró¿n±
warto¶æ:
\begin{itemize}
  ...
  fragment we wzrocu spasowa³ siê z tekstem.\newline
  \newline
...
umo¿liwiaj±ce wyjmowanie i wk³adanie do niej wiadomo¶ci. Reprezentowana jest
...
Modu³ odbieraj±cy odbiera wiadomo¶ci, które maj± trafiæ do filtra to serwer
SMTP, który s³ucha na okre¶lonym porcie przychodz±cych po³±czeñ. Serwer ten nie
obs³uguje autoryzacji. Ka¿de pod³±czenie siê nowego klienta powoduje
uruchomienie kolejnego w±tku odbieraj±cego. Serwer reprezentowany jest przez
klasê SMTPServer, za¶ po³±czenie (w±tek po³±czeniowy) przez klasê
SMTPConnection.
Modu³ wysy³aj±cy to daemon, który sprawdza cyklicznie czy w kolejce wychodz±cej
znajduj± siê wiadomo¶ci. Je¿eli tak jest, wiadomo¶æ usuwana jest z kolejki i
uruchamiany jest klient SMTP, który stara siê wys³aæ wiadomo¶æ. Nie obs³ugiwane
jest ponawianie wysy³ania wiadomo¶æ w przypadku nieudanej próby.
...
\newline
\newline
ant nazwa\_zadania np. ant compile
\newline
...
\begin{tabular}{ | c | c | c | c | c | c | }
\hline
Lp & Odbieranie & Kolejka 1 & Parsowanie & Analiza & Kolejka 2 \\
\hline
\hline
1 & 174 & 2585 & 1 & 7 & 2795 \\
2 & 170 & 330  & 1 & 3 &  -   \\
...
to z usypianiem na 3000 ms daemonów analizuj±cych kolejkê w przypadku braku w
...
Stworzenie filtra protoko³u SMTP nie jest prostym zadaniem. Wymaga obszernej
...
implementacji SMTP ze standardami opisanymi w RFC. Ma to wp³yw na postaæ
wiadomo¶ci przesy³anej przez klienty do filtra i wymusza konieczno¶æ
...
Du¿ym problemem jest tak¿e ¶rodowisko do testowania, szczególnie je¶li chodzi o
testowanie wydajno¶ciowe na du¿± skalê. Musi ono umo¿liwiaæ jednoczesne
